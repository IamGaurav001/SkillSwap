/**
 * /api/gigs -- POST creates a gig, GET lists the marketplace.
 *
 ...
 *   ?search=   case-insensitive substring of the gig TITLE (see lib/filters.ts)
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    return serverError('GET /api/gigs', err);
  }
}
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    // Optional: a gig posted through the API without a creator name is still a
    // valid gig, so this defaults rather than 400s.
    ...
    return serverError('POST /api/gigs', err);
  }
}
